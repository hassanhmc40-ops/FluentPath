...
<div class="max-w-lg">
    ...
    <form method="POST" action="{{ route('admin.quizzes.update', $quiz) }}" class="bg-white rounded-lg shadow p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <textarea id="description" name="description" rows="3"
                      class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 px-3 py-2 border">{{ old('description', $quiz->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>

        <button type="submit" class="bg-indigo-600 text-white px-6 py-2 rounded hover:bg-indigo-700">Update quiz</button>
    </form>
</div>
@endsection
