...
@section('title', 'Edit Exercise Question')
...
<div style="animation: fadein .4s both;">
    <div style="max-width: 720px;">
        <div style="font-size: 13px; color: #8A8378; margin-bottom: 18px;">Editing question · {{ $quizQuestion->quiz->title ?? '' }}</div>

        <form method="POST" action="{{ route('admin.quiz-questions.update', $quizQuestion) }}" style="background: #17211E; color: #EFEAE2; border-radius: 20px; padding: 28px;">
            @csrf
            @method('PUT')
            <div style="font-family: 'Bricolage Grotesque', sans-serif; font-size: 20px; font-weight: 700;">Edit question</div>
            ...
            <label style="display: block; margin-top: 22px;">
                ...
                <select name="quiz_id" required style="width: 100%; border: 1px solid #2C3A35; border-radius: 11px; padding: 12px 14px; font-size: 14px; background: #202D29; color: #EFEAE2; outline: none; cursor: pointer;" onfocus="this.style.borderColor='#29C39F'" onblur="this.style.borderColor='#2C3A35'">
                    @foreach ($quizzes as $quiz)
                        <option value="{{ $quiz->id }}" @selected(old('quiz_id', $quizQuestion->quiz_id) == $quiz->id)>{{ $quiz->title }} — {{ $quiz->lesson?->title ?? 'no lesson' }}</option>
                    @endforeach
                </select>
                @error('quiz_id')
            ...
            <div class="fp-grid-2" style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-top: 16px;">
                @foreach (['a', 'b', 'c', 'd'] as $option)
                    <label style="display: block;">
                        ...
                        <input name="option_{{ $option }}" value="{{ old('option_' . $option, $quizQuestion->{'option_' . $option}) }}" required placeholder="Option {{ strtoupper($option) }}" style="width: 100%; border: 1px solid #2C3A35; border-radius: 11px; padding: 12px 14px; font-size: 14px; background: #202D29; color: #EFEAE2; outline: none;" onfocus="this.style.borderColor='#29C39F'" onblur="this.style.borderColor='#2C3A35'">
                    </label>
                @endforeach
            </div>
            ...
            <label style="display: block; margin-top: 16px;">
                ...
                <select name="correct_answer" required style="width: 100%; border: 1px solid #2C3A35; border-radius: 11px; padding: 12px 14px; font-size: 14px; background: #202D29; color: #EFEAE2; outline: none; cursor: pointer;" onfocus="this.style.borderColor='#29C39F'" onblur="this.style.borderColor='#2C3A35'">
                    @foreach (['a', 'b', 'c', 'd'] as $option)
                        <option value="{{ $option }}" @selected(old('correct_answer', $quizQuestion->correct_answer) === $option)>{{ strtoupper($option) }}</option>
                    @endforeach
                </select>
