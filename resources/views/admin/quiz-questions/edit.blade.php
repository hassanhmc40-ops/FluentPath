...
@section('title', 'Admin — Edit Quiz Question')

@section('content')
<div class="max-w-lg">
    <h1 class="text-2xl font-bold mb-6">Edit Quiz Question</h1>

    <form method="POST" action="{{ route('admin.quiz-questions.update', $quizQuestion) }}" class="bg-white rounded-lg shadow p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select id="quiz_id" name="quiz_id" class="mt-1 block w-full rounded border-gray-300 shadow-sm px-3 py-2 border">
                @foreach ($quizzes as $quiz)
                    <option value="{{ $quiz->id }}" @selected(old('quiz_id', $quizQuestion->quiz_id) == $quiz->id)>{{ $quiz->title }} — {{ $quiz->lesson->title }}</option>
                @endforeach
            </select>
            @error('quiz_id')
        ...
        <div class="mb-4">
            ...
            <textarea id="question" name="question" rows="2"
                      class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 px-3 py-2 border" required>{{ old('question', $quizQuestion->question) }}</textarea>
            @error('question')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            @foreach (['a', 'b', 'c', 'd'] as $option)
                <div>
                    ...
                    <input type="text" id="option_{{ $option }}" name="option_{{ $option }}" value="{{ old('option_' . $option, $quizQuestion->{'option_' . $option}) }}"
