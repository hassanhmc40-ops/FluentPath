...
<div style="animation: fadein .4s both;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
        <div style="font-size: 13px; color: #8A8378;">
            @if ($trashed)
                Trash · {{ $lessons->count() }} {{ Str::plural('deleted lesson', $lessons->count()) }}
            @else
                {{ $lessons->count() }} {{ Str::plural('lesson', $lessons->count()) }} in catalog · admin only
            @endif
        </div>
        <div style="display: flex; gap: 8px; align-items: center;">
            @if ($trashed)
                <a href="{{ route('admin.lessons.index') }}" style="border: 1px solid #E0D8CC; background: none; border-radius: 999px; padding: 11px 20px; font: inherit; font-size: 13px; font-weight: 600; color: #17211E; text-decoration: none; transition: border-color .2s;" onmouseover="this.style.borderColor='#17211E'" onmouseout="this.style.borderColor='#E0D8CC'">← Back to catalog</a>
            @else
                <a href="{{ route('admin.lessons.index', ['trashed' => 1]) }}" style="border: 1px solid #E0D8CC; background: none; border-radius: 999px; padding: 11px 20px; font: inherit; font-size: 13px; font-weight: 600; color: #17211E; text-decoration: none; transition: border-color .2s;" onmouseover="this.style.borderColor='#17211E'" onmouseout="this.style.borderColor='#E0D8CC'">Trash ({{ $trashedCount }})</a>
    ...
    @if ($trashed)
        <div style="background: #FFFDFA; border: 1px solid #E5DDD2; border-radius: 20px; overflow: hidden;">
            <div style="display: grid; grid-template-columns: 2fr 1fr 90px 120px 120px; gap: 16px; padding: 15px 24px; border-bottom: 1px solid #EDE7DE; font-size: 10.5px; letter-spacing: 1.4px; text-transform: uppercase; color: #8A8378;">
                <div>Title</div><div>Skill</div><div>Level</div><div>Deleted</div><div></div>
            </div>
            @forelse ($lessons as $lesson)
                <div style="display: grid; grid-template-columns: 2fr 1fr 90px 120px 120px; gap: 16px; padding: 16px 24px; border-bottom: 1px solid #F1ECE3; align-items: center; font-size: 13.5px; animation: rise .45s {{ 0.05 + $loop->index * 0.05 }}s both; transition: background .2s;" onmouseover="this.style.background='#FBF8F3'" onmouseout="this.style.background='transparent'">
                    <div style="font-weight: 500; color: #8A8378; text-decoration: line-through;">{{ $lesson->title }}</div>
                    ...
                    <div style="font-family: 'IBM Plex Mono', monospace; font-size: 12px; color: #8A8378;">{{ $lesson->deleted_at?->format('M j, Y') }}</div>
                    <div style="display: flex; gap: 8px; justify-content: flex-end;">
                        <form method="POST" action="{{ route('admin.lessons.restore', $lesson) }}">
                            @csrf
                            @method('PATCH')
                            <button type="submit" style="border: 1px solid #E0D8CC; background: none; border-radius: 999px; padding: 6px 13px; font: inherit; font-size: 12px; color: #0E6B5C; cursor: pointer; transition: border-color .2s;" onmouseover="this.style.borderColor='#0E6B5C'" onmouseout="this.style.borderColor='#E0D8CC'">Restore</button>
                        </form>
                    </div>
                </div>
            @empty
                <div style="padding: 40px 24px; text-align: center; color: #8A8378; font-size: 13px;">Trash is empty.</div>
            @endforelse
        </div>
    @else
        <div style="background: #FFFDFA; border: 1px solid #E5DDD2; border-radius: 20px; overflow: hidden;">
            ...
            @forelse ($lessons as $lesson)
                <div style="display: grid; grid-template-columns: 2fr 1fr 90px 100px 170px; gap: 16px; padding: 16px 24px; border-bottom: 1px solid #F1ECE3; align-items: center; font-size: 13.5px; animation: rise .45s {{ 0.05 + $loop->index * 0.05 }}s both; transition: background .2s;" onmouseover="this.style.background='#FBF8F3'" onmouseout="this.style.background='transparent'">
                    ...
                    <div style="display: flex; gap: 8px; justify-content: flex-end;">
                        ...
                        <form method="POST" action="{{ route('admin.lessons.destroy', $lesson) }}" onsubmit="return confirm('Move this lesson to trash?')">
                            @csrf
                            @method('DELETE')
