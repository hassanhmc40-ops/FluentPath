...
<div style="animation: fadein .4s both;">
    <div style="max-width: 720px;">
        <div style="font-size: 13px; color: #8A8378; margin-bottom: 18px;">Editing placement question · {{ $placementQuestion->level->value }} · {{ $placementQuestion->skill->value }}</div>

        <form method="POST" action="{{ route('admin.placement-questions.update', $placementQuestion) }}" style="background: #17211E; color: #EFEAE2; border-radius: 20px; padding: 28px;">
            @csrf
            @method('PUT')
            <div style="font-family: 'Bricolage Grotesque', sans-serif; font-size: 20px; font-weight: 700;">Edit placement question</div>
                ...
                <label style="display: block;">
                    ...
                    <select name="skill" required style="width: 100%; border: 1px solid #2C3A35; border-radius: 11px; padding: 12px 14px; font-size: 14px; background: #202D29; color: #EFEAE2; outline: none; cursor: pointer;" onfocus="this.style.borderColor='#29C39F'" onblur="this.style.borderColor='#2C3A35'">
                        @foreach ($skills as $skill)
                            <option value="{{ $skill->value }}" @selected(old('skill', $placementQuestion->skill->value) === $skill->value)>{{ ucfirst($skill->value) }}</option>
                        @endforeach
                    </select>
                    @error('skill')
                        <div style="color: #E0603B; font-size: 12px; margin-top: 6px;">{{ $message }}</div>
            ...
            <div id="mcq-fields" style="margin-top: 20px; display: none;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                    @foreach (['a' => 'Option A', 'b' => 'Option B', 'c' => 'Option C', 'd' => 'Option D'] as $letter => $label)
                        <label style="display: block;">
                            ...
                            <input type="text" name="option_{{ $letter }}" value="{{ old('option_' . $letter, $placementQuestion->{'option_' . $letter} ?? '') }}" maxlength="255" placeholder="{{ $label }} text" style="width: 100%; border: 1px solid #2C3A35; border-radius: 11px; padding: 11px 14px; font-size: 14px; background: #202D29; color: #EFEAE2; outline: none;" onfocus="this.style.borderColor='#29C39F'" onblur="this.style.borderColor='#2C3A35'">
                ...
                <label style="display: block; margin-top: 12px; max-width: 340px;">
                    ...
                    <select name="correct_answer" style="width: 100%; border: 1px solid #2C3A35; border-radius: 11px; padding: 11px 14px; font-size: 14px; background: #202D29; color: #EFEAE2; outline: none; cursor: pointer;" onfocus="this.style.borderColor='#29C39F'" onblur="this.style.borderColor='#2C3A35'">
                        @foreach (['a' => 'Option A', 'b' => 'Option B', 'c' => 'Option C', 'd' => 'Option D'] as $letter => $label)
                            <option value="{{ $letter }}" @selected(old('correct_answer', $placementQuestion->correct_answer ?? '') === $letter)>{{ $label }}</option>
                        @endforeach
                    </select>
